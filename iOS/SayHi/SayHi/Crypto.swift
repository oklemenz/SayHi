...
class Crypto {
    ...
    static var keySize = kCCKeySizeAES256
    static var blockSize = kCCBlockSizeAES128
    
    static func saltKey(key: String) -> [UInt8] {
    ...
    static func generateRandom(length: Int) -> [UInt8] {
        let data = Data(count: length)
        var localData = data;
        _ = localData.withUnsafeMutableBytes { mutableBytes in
            SecRandomCopyBytes(kSecRandomDefault, data.count, mutableBytes)
        }
        var randomBytes = [UInt8](repeating: 0, count: length)
        data.copyBytes(to: &randomBytes, count: length)
        return randomBytes
    }
